...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_attendance'])) {
        ...
        if ($student) {
            ...
            $studentId = $student['student_id']; // Get the actual student_id (e.g., "STU-2025-0001")

            // Save using student_id
    ...
    if (isset($_POST['delete_attendance'])) {
        ...
        $del = $conn->prepare("DELETE FROM student_attendance WHERE id = ?");
    ...
    if (isset($_POST['update_attendance'])) {
        ...
        $upd = $conn->prepare("UPDATE student_attendance SET status = ?, attendance_time = ? WHERE id = ?");
...
$recentAttendance = $conn->query("SELECT id, student_name, section, attendance_date, attendance_time, status FROM student_attendance ORDER BY attendance_date DESC, attendance_time DESC LIMIT 20");
?>

<!-- CSS Includes for standalone view -->
<link rel="stylesheet" href="../CSS/student_table.css">
<link rel="stylesheet" href="../css/dashboard.css">

<div class="manual-override-wrapper" style="padding: 20px;">
    ...
    <div style="background: #fff3cd; border: 1px solid #f39c12; border-radius: 8px; padding: 15px; margin: 15px 0;">
        ...
        <p style="font-size: 13px; color: #856404; margin: 0;">Use this to manually add, edit, or delete attendance
            records.
            ...
            <tbody>
                <?php while ($row = $recentAttendance->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <button
                                onclick="showEditModal(<?php echo $row['id']; ?>, '<?php echo $row['status']; ?>', '<?php echo $row['attendance_time']; ?>')"
                                class="btn-add-student"
                                style="background: #f39c12; padding: 6px 12px; font-size: 12px;">✏️</button>
                            <button onclick="deleteAttendance(<?php echo $row['id']; ?>)" class="btn-add-student"
...
</div> <!-- Close manual-override-wrapper -->